...
void Print( const string& str, const deque<Student>& v ) {
    cout << str << endl;
    typedef deque<Student>::const_iterator CIT;
...
int main( void ) {
    ...
    vs.push_front(Student("关羽",25));
    vs.push_front(Student("马超",32));
    vs.push_front(Student("黄忠",42));
    Print("添加节点后:", vs);
    ...
    *it = Student("张竞也",17);
    ...
    // 内部利用"=="进行比较
    IT fit = find(vs.begin(), vs.end(), Student("马超",32));
    if( fit != vs.end() ) {
        vs.erase( fit );
    }
    Print("找到马超并删除后:", vs);
